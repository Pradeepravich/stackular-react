...
const IndustryInfo: FC<Props> = ({ data }) => {
  const navigate = useNavigate();
  const { contentItemsByCategory } = useContentItemsByCategory(
    kontentVariables.industryByCategoryInfoContentType,
    kontentVariables.industryCategoryField,
    kontentVariables.industryTaxonomies
  );
                      ...
                      <span>
                        {data?.elements?.category?.value?.replace(
                          ...
                          <>
                            <span
                              onClick={() =>
                                navigate(PATHS.industries, {
                                  state:
                                    data?.elements?.industry_category?.value[0]
                                      ?.name,
                                })
                              }
                              className="mx-2 cursor-pointer"
                            >
                              {data?.elements?.industry_category?.value[0]?.name}
                            </span>
                            /
                          </>
                        ) : (
